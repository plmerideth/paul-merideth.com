<?php
    if(!defined('BASE_URL'))
    {
        require('/home/paulme9/public_html/includes/config.inc.php');
	...
	if($_SERVER['REQUEST_METHOD']=='POST')
	{
		...
		if(!empty($_POST['email']))
		{
			...
			{
				echo '<p class="error">ERROR:  The submitted email address does not match any on file.</p>';
			}
		}
		else
		{
		...
		if($uid)
		{
			...
			{
				echo '<p class="error">ERROR:  Your password could not be changed due to a system error.  We apologize for any inconvenience</p>';
			}
		}
		else
		{
...
<!-- Show HTML Form -->
<h1 class="loginForm">Reset Your Password</h1><br/>
<form action="index.php?p=resetPW"method="post">
	<fieldset class="fieldsetLogin">
		<!-- <p class="mainText"><b>Email Address:</b> -->
        <label class="formLabels" for="email"><b>Email Address:</b></label>
			<input type="text" name="email" class="classInput" size="30" maxlength="60"
				value="<?php if(isset($_POST['email'])){echo $_POST['email'];}?>"
			/>
		</p>
        <div align="center"><input type="submit" class="submitLogin" name="submit" value="Reset Password"/></div>
	</fieldset>
